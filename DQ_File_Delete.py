...
#
# DQ_File_Cleanup.py -s C:\Users\lees\Desktop\env\archive -r ".*_[0-9]{8}_[0-9]{4}_[0-9]{4}\.zip$" -l C:\Users\lees\Desktop\env\log -d 7
#
#
##############################################################################################################################
...
def main(argv):
    ...
    try:
          opts, args = getopt.getopt(argv,"hd:s:r:l:t")
    ...
    if DAYS_TO_KEEP=='' or SOURCE_FILE_DIR=='' or FILENAME_REGEX=='' or LOGFILE_DIR=='':
        print 'Required arguments blank'
        print_usage()             
    ### LOG FILE VARIABLES #######################################################################################################
    LOGFILENAME=LOGFILE_DIR + 'DQ_File_Cleanup_' + YYYYMMDDSTR + '.log'
    ...
    if source_dir_list:
       ...
       for filename in source_dir_list:
          ...
          if FILE_IS_OLD:
              ...
              #os.remove(os.path.join(SOURCE_FILE_DIR, filename))
              FILECOUNT+=1

          
       add_log_entry(LOGFILE,'FILES REMOVED', str(FILECOUNT))
    else:
       add_log_entry(LOGFILE,'CLEANUP', 'Nothing to cleanup')
