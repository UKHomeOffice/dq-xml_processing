...
def main(argv):
    ...
    try:
          opts, args = getopt.getopt(argv,"dr:b:s:")
    ...
    SOURCE_FILE_DIR=os.path.join(ROOT_DIR, 'adt_output/')
    
    FILE_COUNTER=0

    ### LOG FILE VARIABLES #######################################################################################################
    LOGFILENAME=LOGFILE_DIR + 'DQ_IL2_DB_GP_LOAD_VOYAGE_EXT_' + YYYYMMDDSTR + '.log'
    ...
    ##############################################################################################################################
    print '\n*** Exit if no files'
    source_dir_list = [f for f in os.listdir(SOURCE_FILE_DIR) if f.endswith('.csv')]
    ...
    if RETURN_CODE not in (0,1):
        print DOS_BATCH_FILE + ' failed' + str(RETURN_CODE)
