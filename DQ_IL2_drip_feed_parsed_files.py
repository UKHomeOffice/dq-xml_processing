...
#DQ_IL2_drip_feed_parsed_files.py -i "C:/Users/lees.DQDEV/Desktop/env/archive/" -o "C:/Users/lees.DQDEV/Desktop/env/nrt_input/" -l "C:/Users/lees.DQDEV/Desktop/env/log/" ^
#-s "20150427_1000_0001" -t 2 -n 20
...
def main(argv):
   ...
   ### REGEX VARIABLES ##########################################################################################################
   PARSED_REGEX=        r'^PARSED_[0-9]{8}_[0-9]{4}_[0-9]{4}.*\.zip$'
   ...
   LOGFILENAME=LOGFILE_DIR + 'DQ_IL2_drip_feed_parsed_files_' + YYYYMMDDSTR + '.log'
   ...
   while True:
       GLOBAL_COUNTER=0
       from_dir_list =  [f for f in os.listdir(FROM_DIR) if (re.match(PARSED_REGEX, f))]
       from_dir_list.sort()
       to_dir_list = [f for f in os.listdir(TO_DIR) if (re.match(PARSED_REGEX, f))]
       if len(to_dir_list) >= MAXBATCHSIZE:
          ...
          for filename in from_dir_list:
              filedateseq =  int(re.sub("_","",filename[7:25]))
